...
void GUIComponent::InitGarageTab() {
	if (ImGui::Button("Garage Button")) {
		Main.Execute([]() {
			Main.SpawnNotification("Garage", "Garage button has been pressed!", 10);
		});
	}
}

void GUIComponent::InitTitlesTab() {
	if (ImGui::Button("Titles Button")) {
		Main.Execute([]() {
			Main.SpawnNotification("Titles", "Titles button has been pressed!", 10);
			});
	}
}
...
void GUIComponent::Render()
{
	...
	if (IsOpen) {
		ImGui::Begin("Roccket League Mod Template", &IsOpen, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize);
		{
